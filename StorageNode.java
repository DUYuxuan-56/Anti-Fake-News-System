...
public class StorageNode {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length != 5) {
			System.err.println("Usage: java StorageNode host port zkconnectstring zknode portforWorker");
			System.exit(-1);
		}
		...
		log.info("Launching server for client");



		int portFE = Integer.parseInt(args[4]);
		log.info("Launching StorageNode node for AA worker on port " + portFE);
		
		// launch Thrift server
		BcryptService.Processor processor2 = new BcryptService.Processor<BcryptService.Iface>(new SignatureServiceHandler());
		TNonblockingServerSocket socket2 = new TNonblockingServerSocket(portFE);
		//THsHaServer.Args sargs = new THsHaServer.Args(socket);
		TThreadedSelectorServer.Args sargs2 = new TThreadedSelectorServer.Args(socket2);
		sargs2.protocolFactory(new TBinaryProtocol.Factory());
		sargs2.transportFactory(new TFramedTransport.Factory());
		sargs2.processorFactory(new TProcessorFactory(processor2));
		//sargs.workerThreads(64);
		TThreadedSelectorServer server2 = new TThreadedSelectorServer(sargs2);
